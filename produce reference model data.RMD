...
#Import data & process
As we analyse our data with two different datasets - one full with unequal number of observations for the behaviors (observed dataset) and the other with equal number of observations per behavior (sampled dataset) - we decided to keep the scripts the same but only change which dataset we import and export.

First chunck for observed dataset
```{r}
...
This chunk for the sampled dataset 
```{r}
# TO TEST/EXPLORE scripts/analyses:
EXPLORE_sample_aggCD <- read.csv(file="EXPLORE-sample-aggCD.csv") 
observed <- EXPLORE_sample_aggCD %>%
  rename(rowID=X) # only necessary if the first column is an X

# FINAL ANALYSIS:
#ANALYZE_sample_aggCD <- read.csv(file="ANALYZE-sample-aggCD.csv") # final analyses
#agg.data <- subset(ANALYZE_sample_aggDC, days=="last") #this is the ANALYZE data for the actual analyses in the final paper
#observed <- agg.data
...
start.time = Sys.time()
        ...
        } #turns empdf with number of matches for each pair of runs being compared
head(empdf)
...
** Note: save the refmodel with the correct name (observed vs sampled data) 

```{r}
# WRITE DATA for full observed data
write.csv(ref.model, file = "EXPLORE-refmodeldata.csv")
```

```{r}
# write data for sampled data
write.csv(ref.model, file = "EXPLORE-sample-refmodeldata.csv")
```
...
```{r}
# file from github
refmodel <- read.csv(file="EXPLORE-refmodeldata.csv") %>% select(-X)  
samplerefmodel <- read.csv(file="EXPLORE-sample-refmodeldata.csv") %>% select(-X)  

# newly generated reference model
head(ref.model) 

setdiff(refmodel, ref.model) # observed data
setdiff(samplerefmodel, ref.model) # sampled data
