...
for (run in 1:replicates) {
  r.seed <- run
  RNGkind(sample.kind = "Rounding") # code to get same random numbers across operating systems (mac vs windows)
  set.seed(r.seed)
...
```{r}
refmodeldata.Sanjay <- read.csv(file="EXPLORE-refmodeldata-SP-3June.csv") %>% select(-X)  # file from github

refmodel.AM <- read.csv(file="EXPLORE-refmodeldata-2June.csv") %>% select(-X)  # file from local environment
head(ref.model) # newly generated reference model

setdiff(refmodeldata.Sanjay, ref.model) 
setdiff(refmodel.AM, ref.model)
