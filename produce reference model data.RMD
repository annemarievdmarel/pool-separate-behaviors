...
```{r}
refmodeldata.Sanjay <- read.csv(file="EXPLORE-refmodeldata-SP-3June.csv") %>% select(-X)  # file from Sanjay's Mac

refmodel.AM <- read.csv(file="EXPLORE-refmodeldata-2June.csv") %>% select(-X)  # file from local environment on Windows laptop
