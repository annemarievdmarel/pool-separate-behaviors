...
head(per0_dyad_aggDC)
colnames(per0_dyad_aggDC)[1] <- "rowID"

# separate the EXPLORE data from the ANALYZE data

agg.data <- subset(per0_dyad_aggDC, days=="first") #this is the EXPLORE data for setting up the code and figuring out all the analyses

#agg.data <- subset(per0_dyad_aggDC, days=="last") #this is the ANALYZE data for the actual analyses in the final paper
...
aggdata.subd.sumXdyad <- aggdata.subd.sumXdyad %>% group_by(actor, subject) %>% summarise(observed.crowd=sum(crowd),
