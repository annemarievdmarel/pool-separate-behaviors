...
EXPLORE_refmodel_summaries <- read.csv(file="EXPLORE-SUMM.refmodel.csv")
...
#strategies summary
EXPLORE_SUMM.refmodel.strategies <- read.csv(file="EXPLORE_SUMM.refmodel.strategies.csv")
```
...
prop.val <- nrow(COR.refmodel[COR.refmodel$COR<COR.observed, ])/nrow(COR.refmodel) 
pval <- ifelse(prop.val>0.5, 1-prop.val, prop.val)

d <- density(COR.refmodel$COR) 
...
abline(v=COR.observed, col="red")
abline(v=COR.refmodel.mean, col="black")
...
unique(EXPLORE_SUMM.refmodel.strategies$strategy.Rcrowd)
unique(EXPLORE_SUMM.refmodel.strategies$strategy.Rdisp)

#summarize
#find number of runs with each strategy
refstrats.crowd <- EXPLORE_SUMM.refmodel.strategies %>% 
                  dplyr::group_by(strategy.Rcrowd) %>% 
                  dplyr::tally()

refstrats.disp <- EXPLORE_SUMM.refmodel.strategies %>% 
...
refstrats <- EXPLORE_SUMM.refmodel.strategies %>% 
