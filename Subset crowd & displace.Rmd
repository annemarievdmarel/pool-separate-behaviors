...
## summarize total number of displacements and crowds per first/last/all days
```{r}
...
## summarize over the whole field season
```{r}
#finds n agg per actor for summarized/trimmed data
dyad.aggDC <- dyad.agg.maxkey %>% 
  group_by(actor, subject, behavior) %>% 
  summarise(total.agg=sum(n.aggXaggXkey))
head(dyad.aggDC)

#cast by aggressive behavior

dyad.agg.cst <- dcast(dyad.aggDC, actor+subject~behavior, value.var="total.agg")
dyad.agg.cst[is.na(dyad.agg.cst)] <- 0

head(dyad.agg.cst)
dyad.agg.cst$total.DC <- dyad.agg.cst$displace + dyad.agg.cst$crowd

#add column with dyad ID
#dyad.agg.cst$dyad <- paste(dyad.agg.cst$actor, dyad.agg.cst$subject, sep="-")
#rownames(dyad.agg.cst) <- dyad.agg.cst$dyad
#head(dyad.agg.cst)
```