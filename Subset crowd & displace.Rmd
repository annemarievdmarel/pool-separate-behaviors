...
```
Another way using tidyverse function pivot_wider
```{r}
dyad.aggDCXday.wide <- dyad.aggDCXday %>% pivot_wider(names_from = "behavior", 
  values_from = "n.aggXday",
  values_fill = list(n.aggXday = 0)) %>%
  mutate(total.DC=displace + crowd)
head(dyad.aggDCXday.wide)
```


## filter periods
first, only 3 days of fitness period for exploratory analysis
```{r}
# select fitness period
head(obs.day.KEY)
fitness <- subset(obs.day.KEY, period=="pre") %>%
  select(date, period)

# combine 
fitness.dyad.aggDC<-right_join(dyad.aggDCXday.cst, fitness, by="date")
str(fitness.dyad.aggDC)
```
...
## select only last 3 days for final analyses
agg.data <- subset(per0.dyad.aggDC, days=="last") %>%
  select(-period, -days) %>%
...
write.csv(fitness.dyad.aggDC, paste0(machine, path.out,"EXPLORE.perpre.aggDC.csv"))
write.csv(per0.dyad.aggDC, paste0(machine, path.out,"ANALYZE.per0.aggDC.csv"))
write.csv(agg.data, "ANALYZE.aggDC.csv")
