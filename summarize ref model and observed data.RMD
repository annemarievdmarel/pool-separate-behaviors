...
EXPLORE_refmodelCOR_CxD <- read.csv(file="EXPLORE_refmodelCOR_CxD.csv") #head(EXPLORE_refmodelCOR_CxD)
```
...
NB! Before you run this chunk, change the location you want to save your file to. You find the code at the end of the chunk under the comment #WRITE DATA
...
#WRITE DATA (change with your own location)
dfile.loc <- "C:/Users/Liz/Dropbox/A ACTIVE MSs/2020-05 Pool vs separate behaviors/pool-separate-behaviors/"
#write.csv(observed.summary, file = paste(dfile.loc, "EXPLORE-SUMM.observed.csv", sep=""))	
...
Loop other than strategies takes about 14 min
...
for(run in 1:replicates){
  ...
  run.corr <- subset(EXPLORE_refmodelCOR_CxD, runID==run.code)
...
### add correlation to ref.model dataframe
head(EXPLORE_refmodelCOR_CxD)
EXPLORE_refmodelCOR<-EXPLORE_refmodelCOR_CxD %>%
  select(-X) %>%
  rename(Rcor=COR, run.cor=runID)

refmodel.summary <- left_join(ref.model.summaries,EXPLORE_refmodelCOR, by="run.cor" )
head(refmodel.summary)

#WRITE DATA
dfile.loc <- "C:/Users/Liz/Dropbox/A ACTIVE MSs/2020-05 Pool vs separate behaviors/pool-separate-behaviors/"
write.csv(ref.model.summaries, file = paste(dfile.loc, "EXPLORE-SUMM.refmodel.csv", sep=""))	
...
#re-summarize observed data
observedXdyad <- observed

#finds n event per dyad (if this is not summarizing by actor and subject, use detach::plyr)
observedXdyad <- observedXdyad %>% 
                  dplyr::group_by(actor, subject) %>% 
                  dplyr::summarise(observed.crowd=sum(crowd),
                          observed.displace=sum(displace),
...
# Observed data: how do aggressive strategies compare for displacement-only hierarchies vs crowd-only hierarchies?
Need to make sure that all birds in the group are included in each network (fill no interactions with zeros)
...
observedXdyad.alldyads <- merge(observedXdyad, dyad.list, 
                           ...
                           all.y=TRUE)
head(observedXdyad.alldyads)
str(observedXdyad.alldyads) #should have 420 dyads
...
#head(aggdata.subd.sumXdyad)
  ...
  Obcrowd.density <- n.Obcrowd.dyads/420
  Obdisplace.density <- n.Obdisplace.dyads/420
  ...
  #Triangle transitivity
  Obcrowd.transi <- EloRating::transitivity(obs.crowd.mx, runs = 1000)
  Obcrowd.transi.Pt <- Obcrowd.transi[1]  
  Obcrowd.transi.ttri <- Obcrowd.transi[2]  
  
  # Compute focus & position
  Obfocus.crowd <- dom_focus(obs.crowd.mx)
  Obposition.crowd <- dom_position(obs.crowd.mx)
  Obfp.crowd <- cbind.data.frame(Obfocus.crowd, Obposition.crowd)
  colnames(Obfp.crowd) <- c("focus", "position")
  
  #Compute blur models
  Obblur.crowd <- dom_make_blur_data(obs.crowd.mx)
  
  #Find strategy
  strategy.Obcrowd <- dom_categorize_strategy(data=Obfp.crowd, blur_data=Obblur.crowd)
  ...
  Obcrowd.hier <- cbind.data.frame(
                                  Obcrowd.linearity, 
                                  Obcrowd.steepness, 
                                  Obfocus.crowd, Obposition.crowd,
                                  Obcrowd.prunk.pu, Obcrowd.prunk.dyads,
                                  Obcrowd.transi.Pt, Obcrowd.transi.ttri,
                                  Obfocus.crowd, Obposition.crowd,
                                  strategy.Obcrowd
  ...
  # Compute focus & position
  Obfocus.disp <- dom_focus(obs.disp.mx)
  Obposition.disp <- dom_position(obs.disp.mx)
  Obfp.disp <- cbind.data.frame(Obfocus.disp, Obposition.disp)
  colnames(Obfp.disp) <- c("focus", "position")
  
  #Compute blur models
  Obblur.disp <- dom_make_blur_data(obs.disp.mx)
  
  #Find strategy
  strategy.Obdisp <- dom_categorize_strategy(data=Obfp.disp, blur_data=Obblur.disp)
  ...
  #Triangle transitivity
  ObtotalCD.transi <- EloRating::transitivity(obs.totalCD.mx, runs = 1000)
  ObtotalCD.transi.Pt <- ObtotalCD.transi[1]  
  ObtotalCD.transi.ttri <- ObtotalCD.transi[2]  
  
  # Compute focus & position
  Obfocus.totalCD <- dom_focus(obs.totalCD.mx)
  Obposition.totalCD <- dom_position(obs.totalCD.mx)
  Obfp.totalCD <- cbind.data.frame(Obfocus.totalCD, Obposition.totalCD)
  colnames(Obfp.totalCD) <- c("focus", "position")
  
  #Compute blur models
  Obblur.totalCD <- dom_make_blur_data(obs.totalCD.mx)
  
  #Find strategy
  Obstrategy.totalCD <- dom_categorize_strategy(data=Obfp.totalCD, blur_data=Obblur.totalCD)
  ...
  ObtotalCD.hier <- cbind.data.frame(
                                  ObtotalCD.linearity, 
                                  ObtotalCD.steepness, 
                                  Obfocus.totalCD, Obposition.totalCD,
                                  ObtotalCD.prunk.pu, ObtotalCD.prunk.dyads,
                                  ObtotalCD.transi.Pt, ObtotalCD.transi.ttri,
                                  Obfocus.totalCD, Obposition.totalCD,
                                  ObtotalCD.linearity, 
...
for(run in 1:replicates){
  ...
  #Triangle transitivity
  Rcrowd.transi <- EloRating::transitivity(ref.crowd.mx, runs = 1000)
  Rcrowd.transi.Pt <- Rcrowd.transi[1]  
  Rcrowd.transi.ttri <- Rcrowd.transi[2]  
  ...
  #Triangle transitivity
  Rdisp.transi <- EloRating::transitivity(ref.disp.mx, runs = 1000)
  Rdisp.transi.Pt <- Rdisp.transi[1]  
  Rdisp.transi.ttri <- Rdisp.transi[2]  
