...
aggD.long<-select(observed, -crowd, -total.DC) %>%
  ...
  select(-displace, -rowID)

# check # observations correct
length(aggD.long$date)
...
I decided to use a loop instead. now I specifically loaded stringr and it worked.
...
# full_join only keeps all rows of x (obs.crowd.rep)
sample.obs.CD<-dplyr::full_join(obs.crowd.rep, sample.aggDXday, by=c("runID", "actor", "subject")) %>%
  select(runID, everything()) %>%
  replace_na(list(crowd = 0, displace = 0)) %>%
  mutate(total.DC=displace + crowd)  

# merge works
sample.obs.CD<-merge(sample.aggDXday,obs.crowd.rep,  by=c("runID", "actor", "subject"), all.x = TRUE, all.y = TRUE) %>%
  select(runID, everything()) %>%
  replace_na(list(crowd = 0, displace = 0)) %>%
  mutate(total.DC=displace + crowd) 
