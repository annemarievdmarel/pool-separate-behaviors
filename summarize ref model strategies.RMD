...
```{r}
#reference model data
EXPLORE_sample_refmodel <- read.csv(file="EXPLORE-sample-refmodeldata.csv")
ref.model <- EXPLORE_sample_refmodel #update name here when changing to ANALYZE ref model
head(ref.model)
```



#Find strategies (takes awhile!-approximately 5 hours)
```{r}
...
```{r}
#WRITE DATA
write.csv(ref.model.strategies, file = "EXPLORE_SUMM.refmodel.strategies.csv", sep="")	write.csv(refmodel.blurdata, file = "EXPLORE_SUMM.refmodel.blurs.csv", sep="")
```

SAMPLED
```{r}
# check right refmodel used
sum(ref.model$observed.crowd[ref.model$runID=="run003"])
sum(ref.model$observed.displace[ref.model$runID=="run050"])

write.csv(ref.model.strategies, file = "EXPLORE_SUMM.sample.refmodel.strategies.csv")	
write.csv(refmodel.blurdata, file = "EXPLORE_SUMM.sample.refmodel.blurs.csv")
```
