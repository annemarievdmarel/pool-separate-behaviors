...
# export file
write.csv(sample.obs.CD, file = "EXPLORE-sparse-aggCD_AM.csv")
```

Now, we can summarize our results using this reference model.

```{r}
# load the reference model data for the sparse dataset
...
First, check whether generated csv file on github is the same as newly generated refmodel
```{r}

ref.model <- EXPLORE_sparse_aggCD %>%
  dplyr::select(-X)
head(ref.model) # file from github
head(sample.obs.CD) # newly generated file

setdiff(ref.model, sample.obs.CD)
...
head(COR.refmodel)
write.csv(COR.refmodel, file = "EXPLORE_sparse_refmodelCOR.csv")	
```



#### 3. summarize ref model
The summary results of the observed crowds are calculated in the 'reference model 1.rmd' file and saved in "EXPLORE-SUMM.observed.csv"

```{r}
# import data (only necessary when you only run this part)
EXPLORE_sparse_aggCD <- read.csv(file="EXPLORE-sparse-aggCD.csv") # reference model
ref.model <- EXPLORE_sparse_aggCD %>%
  dplyr::rename(rowID=X)
...
# export data
write.csv(ref.model.summaries, "EXPLORE-SUMM.sparse.refmodel.csv")
```
...
# export data
write.csv(ref.model.strategies, file = "EXPLORE_SUMM.sparse.refmodel.strategies.csv")	
write.csv(refmodel.blurdata, file = "EXPLORE_SUMM.sparse.refmodel.blurs.csv")
```
