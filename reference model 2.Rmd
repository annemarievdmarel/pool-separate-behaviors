...
There are four steps within the loop:
...
obs.dyad.sum.crowd <- obs.crowd %>% group_by(actor, subject) %>% summarize(obs.dyad.crowd = sum(crowd))  # we could make this shorter or use observedXdyad dataframe; I changed OBS to obs to stay consistent with other variable names
...
for (run in 1:replicates) { #run=2
  ...
  ref.model <- rbind.data.frame(ref.model.sparse, ref.data) # only goes for 1 loop, not all 100. code in the chunk below does work, don't see where I made the mistake. 
...
```
## this code runs (no clue why above code doesn't....): 
The goal of this reference model is to assess how sparser displacement data may affect our summary statistics. To do this we subsampled our displacement event data so that the number of displacements was reduced to equal the number of crowds (which were the rarer behavior type in our observations).
...
#check that runs are different from each other
head(subset(ref.model, sampleID=="run001"))
head(subset(ref.model, sampleID=="run002"))
head(subset(ref.model, sampleID=="run003"))
head(subset(ref.model, sampleID=="run011"))
#check that all runs ran as expected
