...
```{r}
# Import data
refmodel.summary<- read.csv("EXPLORE-SUMM.refmodel.csv") # reference model 1
...
# correlation
beanplot(refmodel.summary$run.cor, refmodel2.summary$run.cor, 
...
points(2,observed.summary$obs.cor, col="red", pch=7)
```

# all other plots combined
using par(mfrow=c(3,2)) for the plots with crowds and displacements (6 total), then correlation plot separate.
The distribution of  randomized crowds and displacaments (ref model 1) and sparse sampled displacements (ref model 2)
```{r}
par(mfrow=c(3,2))

# density
beanplot(refmodel.summary$Rcrowd.density, refmodel.summary$Rdisplace.density, refmodel2.summary$Rdisplace.density,  
         main="Density", 
         ...
         xlab = "", names = c("random crowds", "random displace", "sparse displace"),
...
points(3,observed.summary$Obcrowd.density, col="#8da0cb" , pch=7) # crowds
abline(h=observed.summary$ObtotalCD.density, col="red") # add line estimate of totalDC


# linearity
beanplot(refmodel.summary$Rcrowd.linearity, refmodel.summary$Rdisp.linearity, refmodel2.summary$Rdisp.linearity, 
         main="Linearity", 
         ylab="h index", log = "",
...
points(3,observed.summary$Obcrowd.linearity, col= "#8da0cb" , pch=7) # crowds
abline(h=observed.summary$ObtotalCD.linearity, col="red")  # add line estimate of totalDC 


# steepness
beanplot(refmodel.summary$Rcrowd.steepness, refmodel.summary$Rdisp.steepness,refmodel2.summary$Rdisp.steepness,
         main="Steepness", 
         ylab="Dij", las=1, log = "",
...
points(3,observed.summary$Obcrowd.steepness, col= "#8da0cb" , pch=7) # crowds
abline(h=observed.summary$ObtotalCD.steepness, col="red")  # add line estimate of totalDC 


# sparseness
beanplot(refmodel.summary$Rcrowd.prunk.pu, refmodel.summary$Rdisp.prunk.pu,refmodel2.summary$Rdisp.prunk.pu,
         main ="Sparseness", 
         ylab="Sparseness", 
         ...
         las=1, xlab = "", names = c("random crowds", "random displace", "sparse displace"),
         col = list(alpha("#8da0cb", 0.6),c(alpha("#fc8d62", 0.6)), alpha("black", 0.6)), 
         border = 'NA', 
         ll = 0.05, beanlinewd = 0.1)
...
points(3,observed.summary$Obcrowd.prunk.pu, col="#8da0cb" , pch=7) # crowds
...
points(3,observed.summary$Obcrowd.transi.ttri, col= "#8da0cb" , pch=7) # crowds
...
## SAMPLED dataset
used one single randomization step (not 100)
...
## separate Plots for observed dataset
# correlation
```{r pressure, echo=FALSE}
# correlation
beanplot(refmodel.summary$run.cor,  
...
ggplot(strategies, aes(x=strategy, y=n, fill=behavior)) +
  ...
  theme_classic() +
  scale_fill_manual(values=c(alpha("#8da0cb", 0.6),alpha("#fc8d62", 0.6), alpha("lightgrey",0.6)))  +    
