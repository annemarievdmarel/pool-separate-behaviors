...
Per ID - plot correlation strenght ref model 1 and ref model 2 
```{r}
par(mfrow=c(1,3))

# out strength
beanplot(refmodel1.summary$Rcrowd.strength[refmodel1.summary$ID=="GGB"], refmodel1.summary$Rdisp.strength[refmodel1.summary$ID=="GGB"], refmodel2.summary$Rdisp.strength[refmodel2.summary$ID=="GGB"], 
         main="out-strength", 
         ylab="", log = "", ylim = ylim,
         xlab = "", names = xlabels, 
  ...
  points(1,observed.ID$Obcrowd.degree, col= crowds, pch=obsval.pch, cex=obsval.size, lwd=2) # crowds
  points(2,observed.ID$Obdisp.degree, col= displace, pch=obsval.pch, cex=obsval.size, lwd=2) # displacements
  points(3,observed.ID$Obcrowd.degree, col= crowds , pch=obsval.pch, cex=obsval.size, lwd=2) # crowds

  points(1,observed.ID$Obcrowd.degree, col= obs.vals, pch=obsval.hlt.pch, cex=obsval.hlt.cex) # crowds
  points(2,observed.ID$Obdisp.degree, col= obs.vals, pch=obsval.hlt.pch, cex=obsval.hlt.cex) # displacements
  points(3,observed.ID$Obcrowd.degree, col= obs.vals , pch=obsval.hlt.pch.plus, cex=obsval.hlt.cex) # crowds

  abline(h=observed.ID$ObtotalCD.degree, col=alpha(obs.vals, pooled.alpha), lwd = linewidth)  # add line estimate of totalDC 



# save plot
dev.print(pdf, file="plot.indvidual.beans.pdf")
